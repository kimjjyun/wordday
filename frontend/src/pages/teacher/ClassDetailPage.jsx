...
import { getClass, bulkCreateStudents, deleteStudent, resetStudentPassword, getDailyCheckin } from '../../api/classes';
...
export default function ClassDetailPage() {
  ...
  const [wbForm,    setWbForm]    = useState({ title: '', week: '', wordsPerDay: '10' });
  ...
  const handleAddWordBook = async () => {
    ...
    await createWordBook({ classId: id, title: wbForm.title, week: Number(wbForm.week), wordsPerDay: Number(wbForm.wordsPerDay) || 10 });
    setWbForm({ title: '', week: '', wordsPerDay: '10' }); setShowAddWb(false); load();
  };
  ...
  const handleResetPassword = async (studentId) => {
    ...
    try { await resetStudentPassword(id, studentId); }
    catch { /* no-op */ }
  ...
  const loadCheckin = () => {
    ...
    getDailyCheckin(id).then(r => setCheckin(r.data)).catch(() => {}).finally(() => setCheckinLoading(false));
  };
      ...
      <div className="pb-8">
        ...
        <div className="mb-8">
          ...
          {(checkin || checkinLoading) && (
            <div className="mb-4 bg-gray-50 rounded-[20px] p-4">
              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-300 mb-3">
                오늘 자습 현황 — {checkin ? `${checkin.filter(s => s.studied).length}/${checkin.length}명` : ''}
              </p>
              {checkinLoading ? (
                  ...
                  {checkin.map((s, i) => (
                    <div key={s.id}>
                      <div className="flex items-center gap-3 py-2.5">
                        <span className={`text-[15px] font-black w-5 text-center ${s.studied ? 'text-black' : 'text-gray-200'}`}>
                          {s.studied ? '○' : '×'}
                        </span>
                        <span className="font-bold text-[14px] tracking-tight flex-1">{s.name}</span>
                        <span className="text-[11px] text-gray-300 font-medium">{s.studentCode}</span>
                        {s.studied && (
                          <span className="text-[11px] font-bold text-gray-400">{s.wordCount}단어</span>
                        )}
                      </div>
                      {i < checkin.length - 1 && <div className="h-px bg-gray-100" />}
                    </div>
                  ))}
                </div>
              )}
                  ...
                  <div className="flex items-center justify-between py-3">
                    ...
                    {confirmStudentId === s.id ? (
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleDeleteStudent(s.id)} disabled={deletingId === s.id}
                          className="text-[11px] font-bold text-black transition disabled:opacity-40">확인</button>
                        <button onClick={() => setConfirmStudentId(null)}
                          className="text-[11px] font-bold text-gray-300 hover:text-black transition">취소</button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleResetPassword(s.id)}
                          disabled={resetingId === s.id}
                          title="비밀번호를 1234로 초기화"
                          className="text-[11px] font-bold text-gray-300 hover:text-black transition disabled:opacity-40 border border-gray-100 rounded-full px-2 py-0.5"
                        >{resetingId === s.id ? '...' : 'PW'}</button>
                        <button onClick={() => setConfirmStudentId(s.id)} disabled={deletingId === s.id}
                          className="text-[11px] font-bold text-gray-300 hover:text-black transition disabled:opacity-40 px-1">삭제</button>
                      </div>
                    )}
                  </div>
        ...
        <div>
          ...
          {showAddWb && (
            <div className="mb-4 bg-gray-50 rounded-[20px] p-4 space-y-2.5">
              ...
              <div className="flex gap-2">
                <input className={inputCls + ' bg-gray-50'} type="number" placeholder="주차" value={wbForm.week} onChange={e => setWbForm(f => ({ ...f, week: e.target.value }))} />
                <input className={inputCls + ' bg-gray-50'} type="number" placeholder="일 단어수" value={wbForm.wordsPerDay} onChange={e => setWbForm(f => ({ ...f, wordsPerDay: e.target.value }))} />
              </div>
                ...
                <div key={wb.id}>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[11px] font-bold uppercase tracking-wider text-gray-300 mt-0.5">{wb.week}주차 · {wb.wordsPerDay ?? 10}단어/일</p>
                    </div>
