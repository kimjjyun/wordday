...
import { getWordBook, addWord, bulkAddWords, importCSV, deleteWord, duplicateWordBook } from '../../api/wordbooks';
import { createTest, getWordBookTests } from '../../api/tests';
...
export default function WordBookPage() {
  ...
  const loadHistory = () => {
    setHistLoading(true);
    getWordBookTests(id).then(r => setTestHistory(r.data)).catch(() => {}).finally(() => setHistLoading(false));
  };

  useEffect(() => { load(); loadHistory(); }, [id]);

  const wordsPerDay  = wb?.wordsPerDay ?? 10;
  const wordCount    = wb?.words?.length ?? 0;
  const totalDays    = wordsPerDay > 0 ? Math.ceil(wordCount / wordsPerDay) : 0;

  const dayStart = (testDay - 1) * wordsPerDay + 1;
  const dayEnd   = Math.min(testDay * wordsPerDay, wordCount);
  ...
  const handleDuplicate = async () => {
    setDupLoading(true);
    try {
      const res = await duplicateWordBook(id);
      navigate(`/teacher/wordbooks/${res.data.id}`);
    } catch { /* no-op */ }
    finally { setDupLoading(false); }
  };

  const handleStartTest = async () => {
    const dayNumber = totalDays > 1 ? testDay : 0;
    const res = await createTest({ classId: wb.classId, wordBookId: id, dayNumber });
  ...
  return (
    <Layout title="WORDDAY" back>
      <div className="pb-8">
        ...
        <div className="pt-2 pb-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-[12px] font-medium text-gray-300 mt-1">
                단어 {wordCount}개 · {wordsPerDay}단어/일 · {totalDays > 0 ? `${totalDays}일치` : '—'}
              </p>
            </div>
            <button
              onClick={handleDuplicate}
              disabled={dupLoading}
              className="text-[11px] font-bold text-gray-400 border border-gray-200 rounded-full px-3 py-1.5 hover:border-gray-400 hover:text-black transition disabled:opacity-40 shrink-0 mt-1"
            >{dupLoading ? '복사 중...' : '복제'}</button>
          </div>
        </div>
        ...
        {/* ── 조회/종례 테스트 시작 ───────────────────── */}
        <div className="mb-6">
          <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-gray-300 mb-3">조회 · 종례 테스트</p>
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {Array.from({ length: totalDays }, (_, i) => i + 1).map(day => {
                  const s = (day - 1) * wordsPerDay + 1;
                  const e = Math.min(day * wordsPerDay, wordCount);
                  return (
                    <button
                      key={day}
                      ...
                      className={`px-3 py-1.5 rounded-full text-[12px] font-bold transition ${
                        testDay === day
                          ? 'bg-black text-white'
                          : 'border border-gray-200 text-gray-400 hover:border-gray-500'
                      }`}
                    >
                      Day {day}
                      <span className={`text-[10px] ml-1 ${testDay === day ? 'text-white/60' : 'text-gray-300'}`}>
                        {s}–{e}
                      </span>
                    </button>
                  );
              ...
              <button
                ...
                disabled={!wordCount}
                className="w-full bg-black text-white font-bold py-4 rounded-full text-[15px] tracking-tight active:scale-[0.97] transition disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Day {testDay} 조회 시작 ({dayStart}–{dayEnd}번 단어)
              </button>
            </>
          ) : (
            <button
              ...
              className="w-full bg-black text-white font-bold py-4 rounded-full text-[15px] tracking-tight active:scale-[0.97] transition disabled:opacity-40 disabled:cursor-not-allowed"
            >
              조회 테스트 시작 {wordCount > 0 ? `(전체 ${wordCount}개)` : ''}
            </button>
          )}
        </div>

        {/* ── 테스트 히스토리 ─────────────────────────── */}
        {(testHistory.length > 0 || histLoading) && (
          <>
            <div className="h-px bg-gray-100 mb-5" />
            <div className="mb-6">
              ...
              {histLoading ? (
                <div className="flex gap-1.5 py-4">
                  {[...Array(3)].map((_, i) => <div key={i} className="w-1.5 h-1.5 rounded-full bg-gray-200 animate-pulse" />)}
                </div>
              ) : (
                <div>
                  {testHistory.map((t, i) => (
                    <div key={t.id}>
                      ...
                      >
                        <div>
                          <p className="font-bold text-[14px] tracking-tight text-black">
                            {t.dayNumber > 0 ? `Day ${t.dayNumber}` : '전체'}
                            <span className="text-gray-300 font-medium ml-2 text-[12px]">{fmtDate(t.createdAt)}</span>
                          </p>
                          <p className="text-[11px] text-gray-300 font-medium mt-0.5">
                            {t.studentCount}명 참여 · 평균 {t.avg}/{t.total}
                          </p>
                        </div>
        ...
        <div>
          ...
          {wordCount === 0 ? (
            ...
            <div>
              {wb.words.map((w, i) => {
                const dayIdx  = Math.floor(i / wordsPerDay) + 1;
                const isFirst = i % wordsPerDay === 0;
                return (
                  <div key={w.id}>
                    {isFirst && totalDays > 1 && (
                      <p className="text-[10px] font-black uppercase tracking-widest text-gray-200 pt-3 pb-1 ml-8">
                        Day {dayIdx}
                      </p>
                    )}
                    <div className="flex items-center gap-3 py-3">
                      ...
                      {confirmWordId === w.id ? (
                        <div className="flex items-center gap-2 shrink-0">
                          <button onClick={() => handleDeleteWord(w.id)} disabled={deletingId === w.id} className="text-[11px] font-bold text-black transition disabled:opacity-40">확인</button>
                          <button onClick={() => setConfirmWordId(null)} className="text-[11px] font-bold text-gray-300 hover:text-black transition">취소</button>
                        </div>
                      ) : (
                        <button onClick={() => setConfirmWordId(w.id)} disabled={deletingId === w.id} className="text-[11px] font-bold text-gray-300 hover:text-black transition disabled:opacity-40 px-1 shrink-0">×</button>
                      )}
                    </div>
                    {i < wordCount - 1 && <div className="h-px bg-gray-50 ml-8" />}
                  </div>
                );
              })}
            </div>
