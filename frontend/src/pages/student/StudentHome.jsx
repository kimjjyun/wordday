      ...
      <div className="pb-8">
        ...
        <div className="pt-2 pb-5">
          <div className="flex items-center justify-between mb-1">
            <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-gray-300">{dateStr}</p>
            <button
              onClick={() => navigate('/student/settings')}
              className="text-[11px] font-bold text-gray-300 border border-gray-100 rounded-full px-3 py-1 hover:border-gray-400 hover:text-black transition"
            >설정</button>
          </div>
          <h1 className="text-5xl font-black tracking-tighter text-black leading-none">{dayEn}</h1>
          {user?.class?.name && (
            <p className="text-[12px] font-bold text-gray-300 mt-1">{user.class.name} · {user.name}</p>
          )}
          {stats && (
