...
async function createWordBook(req, res, next) {
  try {
    const { classId, title, week, wordsPerDay = 10 } = req.body;
    ...
    const wb = await prisma.wordBook.create({
      data: { title, week: Number(week), wordsPerDay: Number(wordsPerDay), classId },
...
async function duplicateWordBook(req, res, next) {
  try {
    const src = await prisma.wordBook.findUnique({
      where: { id: req.params.id },
      include: { words: { orderBy: { order: 'asc' } } },
    });
    if (!src) return res.status(404).json({ error: '단어장을 찾을 수 없습니다.' });

    const newWb = await prisma.wordBook.create({
      data: {
        title:       src.title + ' (복사)',
        week:        src.week,
        wordsPerDay: src.wordsPerDay,
        classId:     src.classId,
      },
    });
    ...
    res.status(201).json({ id: newWb.id, title: newWb.title });
  } catch (err) {
    next(err);
  }
}

module.exports = { createWordBook, getWordBook, getWords, addWord, bulkAddWords, importCSV, deleteWord, duplicateWordBook };
