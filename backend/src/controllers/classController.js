  ...
  try {
    const cls = await prisma.class.findFirst({
      ...
      include: {
        ...
        wordBooks: { select: { id: true, title: true, week: true, wordsPerDay: true, isActive: true } },
      },
    });
...
async function getDailyCheckin(req, res, next) {
  try {
    const cls = await prisma.class.findFirst({
      where: { id: req.params.id, teacherId: req.user.sub },
      include: { students: { select: { id: true, name: true, studentCode: true } } },
    });
    if (!cls) return res.status(404).json({ error: '학급을 찾을 수 없습니다.' });
    ...
    const studied = await prisma.studyRecord.groupBy({
      by:    ['studentId'],
      where: {
    ...
    const studiedMap = new Map(studied.map(r => [r.studentId, r._count.wordId]));

    res.json(cls.students.map(s => ({
      id:          s.id,
      name:        s.name,
      studentCode: s.studentCode,
      studied:     studiedMap.has(s.id),
      wordCount:   studiedMap.get(s.id) ?? 0,
    })));
  } catch (err) {
    next(err);
  }
...
module.exports = { createClass, getClasses, getClass, bulkCreateStudents, deleteStudent, resetStudentPassword, getDailyCheckin };
